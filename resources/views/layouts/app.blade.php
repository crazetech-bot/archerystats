        ...
        <nav class="flex-1 px-3 py-5 space-y-1 overflow-y-auto">
            ...
            @auth
                ...
                @else
                    ...
                    @endif
                    @if(auth()->user()->role === 'club_admin' && app()->has('currentClub'))
                        <a href="{{ route('settings.club-page') }}" @click="mobileOpen = false"
                           class="nav-item flex items-center gap-3 px-4 py-2.5 rounded-r-xl text-sm font-semibold transition-all
                                  {{ request()->routeIs('settings.club-page*') ? 'nav-active' : 'text-slate-400 hover:text-white hover:bg-white/5' }}">
                            <svg class="h-5 w-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
        ...
        <nav class="flex-1 px-3 py-5 space-y-1 overflow-y-auto">
            ...
            @auth
                ...
                @else
                    ...
                    @endif
                    @if(auth()->user()->role === 'club_admin' && app()->has('currentClub'))
                        <a href="{{ route('settings.club-page') }}"
                           class="nav-item flex items-center gap-3 px-4 py-2.5 rounded-r-xl text-sm font-semibold transition-all
                                  {{ request()->routeIs('settings.club-page*') ? 'nav-active' : 'text-slate-400 hover:text-white hover:bg-white/5' }}">
                            <svg class="h-5 w-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/>
