...
@section('title', 'New State Team')
@section('header', 'New State Team')
@section('subheader', 'Create a new state team')
...
<div class="max-w-2xl mx-auto">
    <form method="POST" action="{{ route('state-teams.store') }}" enctype="multipart/form-data">
        @csrf
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm overflow-hidden" style="border:1px solid #e2e8f0;">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="state" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-400">
                                ...
                                @endforeach
                                <option value="Polis DiRaja Malaysia (PDRM)" {{ old('state') == 'Polis DiRaja Malaysia (PDRM)' ? 'selected' : '' }}>Polis DiRaja Malaysia (PDRM)</option>
                                <option value="Angkatan Tentera Malaysia (ATM)" {{ old('state') == 'Angkatan Tentera Malaysia (ATM)' ? 'selected' : '' }}>Angkatan Tentera Malaysia (ATM)</option>
                                <option value="Majlis Sukan Universiti Malaysia (MASUM)" {{ old('state') == 'Majlis Sukan Universiti Malaysia (MASUM)' ? 'selected' : '' }}>Majlis Sukan Universiti Malaysia (MASUM)</option>
                            </select>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="registration_number" value="{{ old('registration_number') }}" placeholder="e.g. MAA/ST/2024/001"
                               class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-400">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Description</label>
                        <textarea name="description" rows="3" placeholder="Team description, achievements, etc."
                                  class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-400 resize-none">{{ old('description') }}</textarea>
                    </div>
                    <div>
