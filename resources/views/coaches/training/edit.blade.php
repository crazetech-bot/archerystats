...
@section('title', 'Edit Training Session — ' . $coach->full_name)
@section('header', 'Edit Training Session')
@section('subheader', $training->date->format('d M Y') . ' · ' . $coach->full_name)

@section('header-actions')
    <a href="{{ route('coaches.training.show', [$coach, $training]) }}"
...
@section('content')
<form method="POST" action="{{ route('coaches.training.update', [$coach, $training]) }}">
@csrf @method('PUT')
...
<div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="p-6 grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div>
                ...
                <input type="date" id="date" name="date"
                       value="{{ old('date', $training->date->format('Y-m-d')) }}"
            ...
            <div>
                ...
                <input type="text" id="focus_area" name="focus_area"
                       value="{{ old('focus_area', $training->focus_area) }}"
            ...
            <div class="sm:col-span-2">
                ...
                <textarea id="notes" name="notes" rows="3"
                          class="block w-full rounded-xl border border-gray-300 bg-gray-50 text-sm py-2.5 px-4
                                 focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 focus:bg-white outline-none transition resize-none">{{ old('notes', $training->notes) }}</textarea>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden"
         x-data="{ roundSelected: '{{ old('round_type_id', $training->round_type_id ?? '') }}' }">
        <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100"
            ...
            <div>
                ...
                <p class="text-xs text-gray-500">Assign a round type — archer sessions will be updated accordingly</p>
            </div>
        </div>
            ...
            <div>
                ...
                <select id="round_type_id" name="round_type_id" x-model="roundSelected"
                    ...
                    <optgroup label="{{ ucfirst($category) }}">
                        @foreach($types as $rt)
                        <option value="{{ $rt->id }}"
                                {{ old('round_type_id', $training->round_type_id) == $rt->id ? 'selected' : '' }}>
                            {{ $rt->name }}
            ...
            <div x-show="roundSelected !== ''" x-cloak class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" id="distance_meters" name="distance_meters"
                           value="{{ old('distance_meters', $training->distance_meters) }}"
                ...
                <div>
                    ...
                    <input type="number" id="target_face_cm" name="target_face_cm"
                           value="{{ old('target_face_cm', $training->target_face_cm) }}"
                           min="10" placeholder="Leave blank to use round default"
                           class="block w-full rounded-xl border border-gray-300 bg-gray-50 text-sm py-2.5 px-4
                                  focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:bg-white outline-none transition">
                </div>
                <p class="sm:col-span-2 text-xs text-gray-400">
                    Only unscored archer sessions will be updated. Scored sessions are preserved.
                </p>
            </div>
    ...
    @php
        ...
        $existingMatches = $training->eliminationMatches()->with(['archerA.user', 'archerB.user'])->get();
    @endphp
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden"
         x-data="{
             pairs: {!! $initPairs !!},
        ...
        <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100"
            ...
            <div>
                ...
                <p class="text-xs text-gray-500">Assign head-to-head match pairs for this session</p>
            </div>
        </div>
        <div class="p-6 space-y-3">

            {{-- Existing matches (read-only) --}}
            @if($existingMatches->isNotEmpty())
            <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Existing Matches</p>
            ...
            <div class="flex items-center gap-3 border border-gray-200 rounded-xl px-4 py-3 bg-gray-50">
                <span class="px-2 py-0.5 rounded-full text-xs font-bold text-white flex-shrink-0"
                      style="background: {{ $catColor }};">{{ $catLabel }}</span>
                <span class="text-sm font-semibold text-gray-800 flex-1 truncate">{{ $nameA }} vs {{ $nameB }}</span>
                @if($em->status === 'completed')
                    <span class="text-xs font-bold px-2 py-0.5 rounded-lg flex-shrink-0"
                          style="background:rgba(16,185,129,0.12); color:#065f46;">Completed</span>
                @else
                    <span class="text-xs font-semibold px-2 py-0.5 rounded-lg flex-shrink-0"
                          style="background:rgba(245,158,11,0.12); color:#92400e;">In Progress</span>
                @endif
                <a href="{{ route('elimination-matches.scorecard', $em) }}"
                   class="text-xs font-bold text-purple-500 hover:text-purple-700 transition-colors flex-shrink-0">
                    {{ $em->status === 'completed' ? 'View →' : 'Score →' }}
                </a>
            </div>
            @endforeach
            <p class="text-xs text-gray-400 pb-1">To add new matches, use the form below.</p>
            @endif
            ...
            <div x-show="pairs.length === 0"
                 class="py-6 text-center border-2 border-dashed border-purple-200 rounded-xl">
                <p class="text-sm text-gray-400">Click below to add new match pairs.</p>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-6" x-data="{ selected: {{ json_encode(array_values($attendingIds)) }} }">
            <div class="flex items-center justify-between mb-4">
                <p class="text-xs text-gray-500"><span x-text="selected.length"></span> selected</p>
                <button type="button"
                        @click="selected.length === {{ $clubArchers->count() }}
                            ? selected = []
                            : selected = {{ json_encode($clubArchers->pluck('id')->values()) }}"
                        class="text-xs font-medium text-teal-600 hover:text-teal-800 hover:underline">
                    <span x-text="selected.length === {{ $clubArchers->count() }} ? 'Deselect All' : 'Select All'"></span>
                </button>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                ...
                <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 cursor-pointer transition-all"
                    ...
                    <input type="checkbox" name="archer_ids[]" value="{{ $archer->id }}"
                           x-model="selected" :value="{{ $archer->id }}"
