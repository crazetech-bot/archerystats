                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="registration_number" value="{{ old('registration_number', $club->registration_number) }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="founded_year" value="{{ old('founded_year', $club->founded_year) }}" min="1900" max="{{ date('Y') }}"
                                   class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-400">
                        </div>
                    </div>
                    <div>
                        ...
                        <textarea name="description" rows="3"
                                  class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-400 resize-none">{{ old('description', $club->description) }}</textarea>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Status</label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="active" value="1" {{ old('active', $club->active) ? 'checked' : '' }}
            ...
            <div class="bg-white rounded-2xl shadow-sm overflow-hidden" style="border:1px solid #e2e8f0;">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="url" name="website" value="{{ old('website', $club->website) }}" placeholder="https://"
                               class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-400">
                    ...
                    <div>
                        ...
                        <select name="state" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-400">
                            ...
                            @foreach(\App\Models\Archer::MALAYSIAN_STATES as $state)
                                <option value="{{ $state }}" {{ old('state', $club->state) == $state ? 'selected' : '' }}>{{ $state }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="p-6">
                    @if($club->logo_url)
                        <div class="mb-4 flex items-center gap-4">
                            <img src="{{ $club->logo_url }}" alt="Current logo" class="h-16 w-16 object-contain rounded-xl border border-slate-200">
                            <p class="text-xs text-slate-500">Current logo. Upload a new file to replace it.</p>
                        </div>
                    @endif
